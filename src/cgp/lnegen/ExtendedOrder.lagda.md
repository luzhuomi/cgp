...
left->-right-> {l} {r} {loc} {c} {sf} pdi₁ sf∈pdi₁ pdi₂ sf∈pdi₂ = >-pdi (pdinstance-left pdi₁) (pdinstance-right pdi₂) (left-∈ pdi₁ sf∈pdi₁) (right-∈ pdi₂ sf∈pdi₂) ev
  where
    ev : ∀ (u₁ : U (l + r ` loc)) (u₂ : U (l + r ` loc))
      ...
      → proj₁ (flat u₂) ≡ c ∷ sf      
      → (l + r ` loc) ⊢ u₁ > u₂
    ev (LeftU v₁) (RightU v₂) rec₁ rec₂ |u₁|≡c∷sf |u₂|≡c∷sf  =
...
map-star-ex-sorted : ∀ { r : RE } { ε∉r : ε∉ r } { loc : ℕ } { c : Char } { sf : List Char }
                     ...
                     → Ex>-sorted {r * ε∉r ` loc } {c} {sf}  (List.map pdinstance-star pdis) -- pdinstance-star pdi is unrolloing pdi* into pdi , pdi*
                     -- for them to be sorted, we need to make sure pdi , (pdinstance-star pdi) are sorted 
map-star-ex-sorted {r} {ε∉r} {loc} {c} {sf} []  _  = ex>-sorted-nil
map-star-ex-sorted {r} {ε∉r} {loc} {c} {sf} (pdi ∷ [])  bla = {!!} 
