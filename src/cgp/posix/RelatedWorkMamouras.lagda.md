...
{-
rewriting the above using Mamoura's notation
...
we can't show [0, 1] <0^r [0, 1] anyway

because [0, 1] <0 ^r [0, 1] iff min rho-1_i([0, 1]) <_i min rho-1_i([0, 1])

where min rho-1_i([0,1]) is finding the minimal tag where [0, 1] is fixed.

min rho-1_i([0,1]) is left . left 
