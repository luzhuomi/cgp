...
open PartialDerivative using ( pdU[_,_] ; pdUConcat ; PDInstance ; pdinstance ; pdinstance-left ; pdinstance-right;  pdinstance-fst ; mkinjFst ;  pdinstance-snd ; zip-es-flat-[]-es;   mk-snd-pdi ; mkinjSnd ; compose-pdi-with;  advance-pdi*-with-c ; concatmap-pdinstance-snd; pdinstance-star ; mkinjList ; pdinstance-assoc ; mkinjAssoc ; pdUMany[_,_]; pdUMany-aux ;   PDInstance* ; pdinstance*  ) 
...
data _⊢_>_ : ∀ ( r : RE ) → U r → U r → Set where
  -- should not change the spec
  {-
  seq₁-● : ∀ { l s r : RE } { loc₁ loc₂ : ℕ } { v₁ v₁' : U l } { v₂ v₂' : U s } { v₃ v₃' : U r }
    → l ● ( s ● r ` loc₂) ` loc₁ ⊢ PairU v₁ (PairU v₂ v₃) > PairU v₁' (PairU v₂' v₃')
    ---------------------------------------------------------------------------------
    → ( l ● s ` loc₁) ● r ` loc₂ ⊢ PairU (PairU v₁ v₂) v₃ > PairU (PairU v₁' v₂') v₃'
  
  seq₁-+ : ∀ { l s r : RE } { loc₁ loc₂ : ℕ } { v₁ v₁'  : U ( l + s ` loc₁ ) } { v₂ v₂' : U r }
    →   l + s ` loc₁ ⊢ v₁ >  v₁'   
    ------------------------------------------------------------------
    →  ( l + s ` loc₁ ) ● r ` loc₂ ⊢ PairU v₁ v₂ > PairU v₁' v₂'

  seq₁-* : ∀ { l r : RE } { loc₁ loc₂ : ℕ } {ε∉l : ε∉ l } { v₁ v₁'  : U ( l * ε∉l ` loc₁) } { v₂ v₂' : U r }
    →   l * ε∉l ` loc₁ ⊢ v₁ >  v₁'   
    ------------------------------------------------------------------
...
module ExampleAntimirov where
  ...
  t8>t9 : a*+a*●a* ⊢ t8 > t9
  t8>t9 = seq₁ (choice-rl-empty (λ () ) refl )
  ...
  t14>t13 = seq₁ (seq₁ (choice-lr-bothempty refl refl) )  
...
¬[]>[] {l ● r ` loc } {ε∈ ε∈l ● ε∈r } (PairU u₁ u₂) (PairU v₁ v₂) ¬proj₁flat-pair-u₁u₂≡[] proj₁flat-pair-v₁v₂≡[] = {!!} 
  -- case 1: u₁≡v₁ , which means |u₁| ≡ [], ¬ |u₂| ≡ []. I.H. via seq₂
  -- case 2: ¬u₁≡v₁
  -- sub case 2.1: ¬ |u₁| ≡ [], I.H. via seq₁ 
  -- sub case 2.2:   |u₁| ≡ [], how to show u₁>v₁? alternatively, it must be u₁≡v₁?
  --  if we apply assoc 
  --                 
  -- seq₁-empty ¬proj₁flat-pair-u₁u₂≡[] proj₁flat-pair-v₁v₂≡[]
...
|∷|>|[]| {r} {ε∈r} {c} {cs} u v proj₁flatu≡c∷cs proj₁flatv≡[] = ¬[]>[] {r} {ε∈r} u v  ¬proj₁flatu≡[]   proj₁flatv≡[]
  where
    ...
    ¬proj₁flatu≡[] rewrite proj₁flatu≡c∷cs =  Utils.¬∷≡[]
...
map-pairU-empty-sorted  {l} {r} {loc} (u ∷ [])  vs all-flat-[]-u  (>-cons  >-nil >-nothing)   >-sorted-vs rewrite (cong (λ x → >-sorted x) (++-identityʳ (List.map (PairU {l} {r} {loc} u) vs)))  = map-pair-u-vs-sorted u vs >-sorted-vs
  where
