...
>-trichotomy u v = >-total u v
  where
    swap->ⁱ : ∀ { r : RE } { u v : U r }
      → (r ⊢ v > u) ⊎ (r ⊢ u > v) ⊎ (v ≡ u)
      → (r ⊢ u > v) ⊎ (r ⊢ v > u) ⊎ (u ≡ v)
    swap->ⁱ (inj₁ v>u) = inj₂ (inj₁ v>u)
    swap->ⁱ (inj₂ (inj₁ u>v)) = inj₁ u>v
    swap->ⁱ (inj₂ (inj₂ v≡u)) = inj₂ (inj₂ (sym v≡u))

    resolve->ⁱ : ∀ { r : RE } { u v : U r }
      → r ⊢ u >ⁱ v
      → (r ⊢ u > v) ⊎ (r ⊢ v > u) ⊎ (u ≡ v)
    resolve->ⁱ {r} {u} {v} u>ⁱv
      with length (proj₁ (flat u)) Nat.≟ 0
          | length (proj₁ (flat v)) Nat.≟ 0
    ... | yes len|u|≡0 | yes len|v|≡0
      = inj₁ (be (trans len|u|≡0 (sym len|v|≡0)) len|v|≡0 u>ⁱv)
    ... | yes len|u|≡0 | no ¬len|v|≡0
      = inj₂ (inj₁ (lne (¬≡0→>0 ¬len|v|≡0) len|u|≡0))
    ... | no ¬len|u|≡0 | yes len|v|≡0
      = inj₁ (lne (¬≡0→>0 ¬len|u|≡0) len|v|≡0)
    ... | no ¬len|u|≡0 | no ¬len|v|≡0
      = inj₁ (bne (¬≡0→>0 ¬len|u|≡0) (¬≡0→>0 ¬len|v|≡0) u>ⁱv)

    len|ListU-v∷vs|>0 : ∀ { r : RE } { nε : ε∉ r } { loc : ℕ } ( v : U r ) ( vs : List (U r) )
      → length (proj₁ (flat (ListU {r} {nε} {loc} (v ∷ vs)))) Nat.> 0
    len|ListU-v∷vs|>0 {r} {nε} {loc} v vs
      = ¬≡0→>0 (λ { len≡0 → (ε∉r→¬ε∈r nε) (proj₁flat-v≡[]→ε∈r {r} {v}
          (++-conicalˡ (proj₁ (flat v)) (proj₁ (flat (ListU vs)))
            (length≡0→[] len≡0))) })

    ListU-cong : ∀ {r' : RE} {nε' : ε∉ r'} {loc' : ℕ} {us' vs' : List (U r')} {u' v' : U r'}
      → u' ≡ v' → ListU {r'} {nε'} {loc'} us' ≡ ListU vs' → ListU (u' ∷ us') ≡ ListU (v' ∷ vs')
    ListU-cong refl refl = refl

    >-total : ∀ { r : RE } ( u v : U r ) → (r ⊢ u > v) ⊎ (r ⊢ v > u) ⊎ (u ≡ v)

    >-total-ListU : ∀ { r : RE } { nε : ε∉ r } { loc : ℕ } ( us vs : List (U r) )
      → (r * nε ` loc ⊢ ListU us > ListU vs) ⊎ (r * nε ` loc ⊢ ListU vs > ListU us) ⊎ (ListU us ≡ ListU vs)

    >-total {ε} EmptyU EmptyU = inj₂ (inj₂ refl)
    >-total {$ c ` loc} (LetterU c) (LetterU c) = inj₂ (inj₂ refl)

    >-total {l + r ` loc} (LeftU u) (LeftU v)
      with >-total {l} u v
    ... | inj₁ u>v = resolve->ⁱ (choice-ll u>v)
    ... | inj₂ (inj₁ v>u) = swap->ⁱ (resolve->ⁱ (choice-ll v>u))
    ... | inj₂ (inj₂ u≡v) = inj₂ (inj₂ (cong LeftU u≡v))

    >-total {l + r ` loc} (RightU u) (RightU v)
      with >-total {r} u v
    ... | inj₁ u>v = resolve->ⁱ (choice-rr u>v)
    ... | inj₂ (inj₁ v>u) = swap->ⁱ (resolve->ⁱ (choice-rr v>u))
    ... | inj₂ (inj₂ u≡v) = inj₂ (inj₂ (cong RightU u≡v))

    >-total {l + r ` loc} (LeftU u) (RightU v)
      = resolve->ⁱ choice-lr
    >-total {l + r ` loc} (RightU u) (LeftU v)
      = swap->ⁱ (resolve->ⁱ choice-lr)

    >-total {l ● r ` loc} (PairU u₁ v₁) (PairU u₂ v₂)
      with >-total {l} u₁ u₂
    ... | inj₁ u₁>u₂ = resolve->ⁱ (seq₁ u₁>u₂)
    ... | inj₂ (inj₁ u₂>u₁) = swap->ⁱ (resolve->ⁱ (seq₁ u₂>u₁))
    ... | inj₂ (inj₂ u₁≡u₂)
      with >-total {r} v₁ v₂
    ... | inj₁ v₁>v₂ = resolve->ⁱ (seq₂ u₁≡u₂ v₁>v₂)
    ... | inj₂ (inj₁ v₂>v₁) = swap->ⁱ (resolve->ⁱ (seq₂ (sym u₁≡u₂) v₂>v₁))
    ... | inj₂ (inj₂ v₁≡v₂) = inj₂ (inj₂ (cong₂ (PairU {l} {r} {loc}) u₁≡u₂ v₁≡v₂))

    >-total {r * nε ` loc} (ListU us) (ListU vs) = >-total-ListU us vs

    >-total-ListU [] [] = inj₂ (inj₂ refl)
    >-total-ListU [] (v ∷ vs)
      = inj₂ (inj₁ (lne (len|ListU-v∷vs|>0 v vs) refl))
    >-total-ListU (u ∷ us) []
      = inj₁ (lne (len|ListU-v∷vs|>0 u us) refl)
    >-total-ListU {r} (u ∷ us) (v ∷ vs)
      with >-total {r} u v
    ... | inj₁ u>v = resolve->ⁱ (star-head u>v)
    ... | inj₂ (inj₁ v>u) = swap->ⁱ (resolve->ⁱ (star-head v>u))
    ... | inj₂ (inj₂ u≡v)
      with >-total-ListU us vs
    ... | inj₁ us>vs = resolve->ⁱ (star-tail u≡v us>vs)
    ... | inj₂ (inj₁ vs>us) = swap->ⁱ (resolve->ⁱ (star-tail (sym u≡v) vs>us))
    ... | inj₂ (inj₂ us≡vs) = inj₂ (inj₂ (ListU-cong u≡v us≡vs))
