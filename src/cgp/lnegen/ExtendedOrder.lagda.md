...
compose-pdi-with-ex*>-head-map-compose-pdi-with : ∀ { d r : RE } { pf : List Char} { c : Char } { sf : List Char }
  ...
  → ( d-non-prob : ∀ (v₁ v₂ : U d) → proj₁ (flat v₁) ≡ proj₁ (flat v₂) → d ⊢ v₁ ≅ v₂ )
  → ( pdi : PDInstance d c )
  → ( pdis : List (PDInstance d c) )
  → Ex>-maybe pdi (head pdis)
  → PDInstance-accepts-sf pdi sf
  → Maybe-PDInstance-accepts-sf (head pdis) sf
  -------------------------------------------------------------------------------------------------
  → Ex*>-maybe {r} {pf ∷ʳ c} {sf}   (compose-pdi-with d→r s-ev-d→r pdi) (head (List.map (compose-pdi-with d→r s-ev-d→r) pdis))
compose-pdi-with-ex*>-head-map-compose-pdi-with {d} {r} {pf} {c} {sf} c∷sf∈⟦d⟧  d→r s-ev-d→r >-inc-d→r d-non-prob pdi []  ex>-nothing _ _ = ex*>-nothing
compose-pdi-with-ex*>-head-map-compose-pdi-with {d} {r} {pf} {c} {sf} c∷sf∈⟦d⟧  d→r s-ev-d→r >-inc-d→r d-non-prob
  ...
  = ex*>-just (*>-pdi
                             ...
                             (compose-pdi-with d→r s-ev-d→r pdi₂)
                             (*∈-pdi sf∈pdi₁ (λ x → d→r (p₁→d x)) (compose-pdi-with-soundEv d→r s-ev-d→r p₁→d s-ev-p₁→d))
                             (*∈-pdi sf∈pdi₂ (λ x → d→r (p₂→d x)) (compose-pdi-with-soundEv d→r s-ev-d→r p₂→d s-ev-p₂→d))
                              ex*>-ev )
  where
    ex*>-ev : ∀ (u₁ u₂ : U r )
    ...
    ... | recons* {d} {r} {cw₁} {pref} u₁ ( cw₁∈⟦d⟧ , d→r-unflat-cw₁∈⟦d⟧≡u₁ ) | recons* {d} {r} {cw₂} {pref} u₂ ( cw₂∈⟦d⟧ , d→r-unflat-cw₂∈⟦d⟧≡u₂ )
            rewrite sym d→r∘p₁→d-unflat-w₁∈⟦p₁⟧≡u₁ | sym  d→r∘p₁→d-unflat-w₂∈⟦p₂⟧≡u₂ =
                >-inc-d→r v₁ v₂ (d-non-prob v₁ v₂ |v₁|≡|v₂|) (u₁→u₂→rec₁→rec₂→u₁>u₂ v₁ v₂ recons-v₁ recons-v₂)
           where
             v₁ : U d
             v₁ = p₁→d (unflat w₁∈⟦p₁⟧)
             v₂ : U d
             v₂ = p₂→d (unflat w₂∈⟦p₂⟧)
             recons-v₁ : Recons v₁ pdi₁
             recons-v₁ = recons v₁ (w₁∈⟦p₁⟧ , refl)
             recons-v₂ : Recons v₂ pdi₂
             recons-v₂ = recons v₂ (w₂∈⟦p₂⟧ , refl)
             |v₁|≡c∷sf : proj₁ (flat v₁) ≡ c ∷ sf
             |v₁|≡c∷sf =
               begin
                 proj₁ (flat (p₁→d (unflat w₁∈⟦p₁⟧)))
               ≡⟨ s-ev-p₁→d (unflat w₁∈⟦p₁⟧) ⟩
                 c ∷ proj₁ (flat (unflat w₁∈⟦p₁⟧))
               ≡⟨ cong (λ x → c ∷ proj₁ x) (flat∘unflat w₁∈⟦p₁⟧) ⟩
                 c ∷ w₁
               ≡⟨ cong (c ∷_) (sym (proj₂ (∷-inj (++-cancelˡ pf
                 (begin
                   pf ++ c ∷ sf
                 ≡⟨ sym (cong (proj₁ ∘ flat) (sym d→r∘p₁→d-unflat-w₁∈⟦p₁⟧≡u₁)) ⟩
                   proj₁ (flat (d→r (p₁→d (unflat w₁∈⟦p₁⟧))))
                 ≡⟨ s-ev-d→r (p₁→d (unflat w₁∈⟦p₁⟧)) ⟩
                   pf ++ proj₁ (flat (p₁→d (unflat w₁∈⟦p₁⟧)))
                 ≡⟨ cong (pf ++_) (s-ev-p₁→d (unflat w₁∈⟦p₁⟧)) ⟩
                   pf ++ c ∷ proj₁ (flat (unflat w₁∈⟦p₁⟧))
                 ≡⟨ cong (λ x → pf ++ c ∷ proj₁ x) (flat∘unflat w₁∈⟦p₁⟧) ⟩
                   pf ++ c ∷ w₁
                 ∎))))) ⟩
                 c ∷ sf
               ∎
...
map-compose-pdi-with-sorted {d} {r} {pf} {c} {sf} c∷sf∈⟦d⟧ d→r s-ev-d→r >-inc-d→r d-non-prob [] ex>-nil [] = ex*>-nil
map-compose-pdi-with-sorted {d} {r} {pf} {c} {sf} c∷sf∈⟦d⟧ d→r s-ev-d→r >-inc-d→r d-non-prob (pdi ∷ pdis) (ex>-cons pdis-sorted pdi>head-pdis) (sf∈pdi ∷ all-sf) =
  ex*>-cons ind-hyp
  (compose-pdi-with-ex*>-head-map-compose-pdi-with c∷sf∈⟦d⟧ d→r s-ev-d→r >-inc-d→r d-non-prob pdi pdis pdi>head-pdis sf∈pdi maybe-sf∈head-pdis)
  where
    ind-hyp : Ex*>-sorted {r} {pf ∷ʳ c} {sf} (List.map (compose-pdi-with d→r s-ev-d→r) pdis )
    ind-hyp = map-compose-pdi-with-sorted {d} {r} {pf} {c} {sf} c∷sf∈⟦d⟧ d→r s-ev-d→r >-inc-d→r d-non-prob pdis pdis-sorted all-sf
    maybe-sf∈head-pdis : Maybe-PDInstance-accepts-sf (head pdis) sf
    maybe-sf∈head-pdis with pdis | all-sf
    ... | [] | [] = tt
    ... | pdi₂ ∷ _ | sf∈pdi₂ ∷ _ = sf∈pdi₂
