...
{-# OPTIONS --rewriting  #-}
module cgp.robust.greedylne.LNN where
...
open List using (List; _∷_; []; _++_; map; concatMap; _∷ʳ_; length)
   ...
   where
    ...
    ... | c ∷ cs = sub (seq₁ u₁>ᵍu₂)
      where
        ...
        ¬proj₁flat-u₁≡[] rewrite proj₁flat-u₁-eq = λ x → ¬∷≡[] x
        proj₁flat-u₂≡[] : proj₁ (flat u₂) ≡ []
        proj₁flat-u₂≡[] rewrite flat-pair {l} {r} {loc} {u₂} {v₂} = ++-conicalˡ (proj₁ (flat u₂)) (proj₁ (flat v₂)) (CGPUtils.length≡0→[] len|u₂v₂|≡0)
        u₁>ᵍu₂ : l ⊢ u₁ >ᵍ u₂
    ...
    ... | [] = sub (seq₂ u₁≡u₂ v₁>ᵍv₂)
      where
        ...
        proj₁flat-v₂≡[] : proj₁ (flat v₂) ≡ []
        proj₁flat-v₂≡[] rewrite flat-pair {l} {r} {loc} {u₂} {v₂} = ++-conicalʳ (proj₁ (flat u₂)) (proj₁ (flat v₂)) (CGPUtils.length≡0→[] len|u₂v₂|≡0)
  ...
  where
    ...
    prf (LeftU u₁) (LeftU u₂) = to-ev , from-ev
      where
        ...
        from-ev (lne len|u₁|>0 len|u₂|≡0) = Nullary.contradiction (proj₁flat-v≡[]→ε∈r (CGPUtils.length≡0→[] len|u₂|≡0)) (ε∉r→¬ε∈r ε∉l)
        from-ev (be len|u₁|≡len|u₂| len|u₂|≡0 _ ) = Nullary.contradiction (proj₁flat-v≡[]→ε∈r (CGPUtils.length≡0→[] len|u₂|≡0)) (ε∉r→¬ε∈r ε∉l)
    ...
    prf (RightU u₁) (RightU u₂) = to-ev , from-ev
      where 
        ...
        ... | iso rob-r-ev = sub (choice-rr (lnn→∷>ᵍ[] lnn-r u₁ u₂ (CGPUtils.length>0→¬≡[] len|u₁|>0)  (length≡0→[] len|u₂|≡0)) )
...
### Is LNN necessary for Greedy LNE Isomophism?
