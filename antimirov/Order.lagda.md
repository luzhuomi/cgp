...
My attempt to define monomial-tail (2nd exp in a monomial)  as property

```agda

data Mono :  RE  → Set where
  mono-ε : Mono ε
  mono-● : ∀ { l r : RE } { loc : ℕ } 
    → Mono l
    ---------------------------
...
### Lemma : all the partial derivatives are monomial-tails. 

```agda
data MonoPd : { r : RE } { c : Char } → PDInstance r c → Set where
  mono-pd : { p r : RE }     -- ^ partial derivative p and input re r 
              { c : Char }     -- ^ the letter
            → Mono p 
            → ( inj : U p → U r ) -- ^ the injection function 
            → ( s-ev :  ∀ ( u : U p ) → ( proj₁ ( flat {r} (inj u) ) ≡ c ∷ ( proj₁ (flat {p} u) )) )
            → MonoPd (pdinstance {p} {r} {c} inj s-ev)  
```
...
map-pairU-empty-sorted  {l} {r} {loc} (u ∷ u' ∷ us)  vs (flat-[] u flat-u≡[] ∷ flat-[] u' flat-u'≡[] ∷ all-flat-[]-us) (>-cons >-sorted-uus (>-just u>u'))  >-sorted-vs
  ...
  where
    ...
    combine {u} {u'} {t ∷ []} {us} {v ∷ vs} u>u'  (flat-[] _ flat-u≡[]) (flat-[] _ flat-u'≡[]) (>-cons >-sorted-map-pair-u-ts u-t>head-map-pair-u-ts)  >-sorted-ys = 
      >-cons  >-sorted-ys (>-just (seq₁ u>u') ) -- (seq₁sameword (trans flat-u≡[] (sym flat-u'≡[])) u>u'))
